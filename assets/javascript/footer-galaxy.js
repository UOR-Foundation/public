let scene, camera, controls, fov, ratio, near, far, shadow, back, light, renderer,
        ...
        function onChange(params) {
            ...
            for (let i = 0; i < points.length; i++) {
                let point = points[i];
                let distance = Math.sqrt(point.x ** 2 + point.y ** 2);
                let pow = Math.max(0, ((params.radius * 0.8) - distance) / (params.radius * 0.8));
                pow = (1 - Math.cos(pow * Math.PI)) * params.bulge;

                // Main vertex
                positions.push(point.x, point.y, (-dz + (dz * 2) * Math.random()) * pow);
                colors.push(pow, pow, 1);

                let t = Math.round(pow * 5);
                for (let n = 0; n < t; n++) {
                    let vx = point.x - dx + Math.random() * (dx * 2);
                    let vy = point.y - dy + Math.random() * (dy * 2);
                    let vz = (-dz + (dz * 2) * Math.random()) * pow;
            ...
            geometry.computeBoundingSphere(); // Optimizes rendering
